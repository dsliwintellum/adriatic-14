...
BASE = 'about:blank'
APP_DIR = Path('/mnt/data/adriatic14_v103')
CDP = 'http://127.0.0.1:9223'
OUT = Path('/mnt/data/adriatic14_v103/docs')
...
assert evaluate("document.querySelectorAll('[data-trip-shortcut]').length") >= 4
...
# Adaptive review: rating a flashcard Again should rebuild today's review summary.
evaluate("location.hash='#/practice/flashcards'")
wait_for("document.querySelector('[data-rate-phrase][data-rating=\"0\"]') !== null")
evaluate("document.querySelector('[data-rate-phrase][data-rating=\"0\"]').click()")
evaluate("location.hash='#/practice'")
wait_for("document.body.innerText.includes('from struggles')")
assert evaluate("document.querySelector('#view').innerText.includes('1 from struggles')")

# Trip-mode shortcuts surface phrases and role-plays for each requested stop.
for key in ['perast','zabljak','korcula','dubrovnik']:
    evaluate("location.hash='#/trip'")
    wait_for(f"document.querySelector('[data-trip-shortcut=\"{key}\"]') !== null")
    evaluate(f"document.querySelector('[data-trip-shortcut=\"{key}\"]').click()")
    wait_for("document.querySelector('#trip-mode-panel') !== null")
    assert evaluate("document.querySelectorAll('#trip-mode-panel .phrase-card').length") > 0
    assert evaluate("document.querySelectorAll('#trip-mode-panel [data-trip-scenario]').length") > 0
